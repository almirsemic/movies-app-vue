  ...
  <div>
    <div class="watchlist_ratings_favorites">
      ...
      <div class="watchlists">
        ...
        ></strong>
        <div class="movei_and_tvShows">
          <a @click="watchlist = true">Movie <br /></a>
          <a @click="watchlist = false">TV Shows</a>
        </div>
      </div>
    </div>
    <div class="container">
      <div class="watchlist">
        <div class="left_menu">
          <h2>My Watchlist</h2>
          <span v-if="movieWatchlist" @click="watchlist = true"
            >Movie {{ movieWatchlist.length }}
            <hr v-if="watchlist"
          /></span>
          <span v-if="tvWatchlist" @click="watchlist = false"
...
export default {
  ...
  async created() {
    ...
    const moviesWatchlistResponse = await axios.get(
      `account/${account.id}/watchlist/movies?api_key=ffebf14b46dcd2b2bb0af17fdfffaa0c&session_id=${sessionResponse.data.session_id}`
    );
    this.movieWatchlist = moviesWatchlistResponse.data.results;
    const tvWatchlistResponse = await axios.get(
      `account/${account.id}/watchlist/tv?api_key=ffebf14b46dcd2b2bb0af17fdfffaa0c&session_id=${sessionResponse.data.session_id}`
    );
