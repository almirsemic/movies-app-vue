<template>
  <div>
    ...
    <TopBilledCast moviesOrTvEndpoint="tv"></TopBilledCast>
     <div class="logo_and_data" v-if="tvDetails">
      ...
      <img class="network_logo" :src="'https://themoviedb.org/t/p/original/'+ network.logo_path" v-for="network in tvDetails.networks" :key="network.id" v-show="network.logo_path != null">
      <p class="keyword_title">
        Type
      </p>
      <p>{{ tvDetails.type }}</p>
   ...
   <bdi>Current Season</bdi>
    <div class="season" v-if="tvDetails">
      <img :src="'https://themoviedb.org/t/p/original/'+ tvDetails.seasons[0].poster_path">
       <img src="@/assets/nophoto.jpg" v-if="tvDetails.seasons[0].poster_path == null">
    <bdi @click="transitionToEpisodes(tvDetails.id, tvDetails.seasons[0].season_number)">{{ tvDetails.seasons[0].name}}</bdi><br>
    <a v-if="tvDetails.seasons[0].air_date"> {{ tvDetails.seasons[0].air_date.slice(0, 4)}} | {{ tvDetails.seasons[0].episode_count}} Episodes</a>
    <br><small class="premiered">{{ tvDetails.seasons[0].name }} of The Walking Dead premiered on {{ tvDetails.seasons[0].air_date }}</small>
     <p><small>
         {{ tvDetails.seasons[0].overview }}
         </small>
     </p>
    </div>
    <p class="view_all_seasons" @click="transitionToSeasons(tvDetails.id)">View All Seasons</p>
    <hr>
    <Reviews moviesOrTvEndpoint="tv"></Reviews>
  </div>
</template>
...
transitionToEpisodes(id, season_number){
      this.$router.push({ name: "episodes", params: { id, number: season_number } });
