...
<p class="numberOfReviews">Reviews ({{ Object.keys(reviews).length }})</p>
    ...
    <button v-if="Object.keys(reviews).length != 0" type="button" class="btn btn-secondary" @click="transitionToAllReviews(id)">
      Show All Reviews
    </button>
...
methods:{
     ...
     this.$router.push({ name: 'reviewsReadMore', params: { id, type: this.moviesOrTvEndpoint } })
   },
   transitionToAllReviews(id){
     this.$router.push({ name: 'reviewsReadAll', params: { id, type: this.moviesOrTvEndpoint } })
   },
},
created(){
    const ApiKey = "ffebf14b46dcd2b2bb0af17fdfffaa0c";
      ...
      fetch(
      `https://api.themoviedb.org/3/${this.moviesOrTvEndpoint}/${this.$route.params.id}/reviews?api_key=${ApiKey}`
    )
      .then((data) => {
        return data.json();
      })
